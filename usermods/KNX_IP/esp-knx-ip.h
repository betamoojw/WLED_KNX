...
// Debug logging control: define KNX_DEBUG and KNX_UDP_DEBUG at build time to enable verbose logs
...
#  define KNX_UDP_LOG(fmt, ...) do { sendUdpDebug("[KNX][UDP] " fmt, ##__VA_ARGS__); } while(0)
#else
